...
execute as @a[tag=inGame,tag=!respawning,scores={bin.deadGamemode=0}] store result score @s gm.y-location run data get entity @s Pos[1]
execute as @a[tag=inGame,tag=!respawning,scores={bin.deadGamemode=0}] if score @s gm.y-location <= killY gm.general run function kitpvp:gamemode/utility/death/environmental-death

execute if score @s musicCategory matches 2 run scoreboard players set @s musicCategory 1
